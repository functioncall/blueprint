...
## Messages (kind = line style + arrowhead, NEVER colour)
- **Sync call** — solid line, filled triangle head. **Return** — dashed line, open chevron.
- **Self-call** — a small loop on the lifeline (text on whichever side has room).
- **Note** — folded-corner box; an aside, not a message.
- **Label** — text rides ABOVE the line (1 line) or STRADDLES it (2 lines); space-aware wrap, a
  parenthetical drops below; long labels reveal in full on click. No opaque plate.
...
Cardinal rule: **never spend the colour channel on two meanings at once.** Kind → line style;
identity → bar/header hue (positional); a *variable* → a lens with a legend.
...
## The one-line rule
Reaching for a new colour? Stop: is it structure (→ neutral/zebra), kind (→ line style), identity
(→ the positional hue), or a *new variable* (→ a lens with a legend)? There is no fifth reason.
